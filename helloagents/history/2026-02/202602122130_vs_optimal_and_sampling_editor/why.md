...
- 最理想的情况：**能自动识别/锁定棋盘位置**（窗口缩放也能跟着对齐），校准/采样只做兜底。
...
3. **优先自动对齐**：参考 `ref/tetrio-plus/` 的做法（在 tetr.io 的主世界里挂钩 PIXI/渲染层），拿到“本地棋盘”的真实位置与缩放，叠加层跟着它走，尽量不需要你手动校准。
4. 采样点管理（兜底）：新增“采样点管理页”，可视化编辑 `scaleSamples`（删/禁用/导入导出）。仅当自动对齐失败时才用到。
5. 校准体验改进（兜底）：校准模式拖动框实时预览；“保存校准”和“保存为样本”拆开。
6. 叠加提示支持显示“上边界之外”的方块格子（上方缓冲区）。
   - 同样：**详情页里的棋盘渲染**也要能显示上方缓冲区的格子（别裁掉）。
7. 增加快捷键一键开关提示（默认 `E`，避免在输入框打字时误触发）。
8. 记忆系统按 `读取更长块序` 开关分离成两套，互不影响（避免 undo/hold 后出现“同局面不同建议”的错觉）。
