...
- 详情页右上角新增「复制调试包」：一键复制当前状态 + 引擎信息 + pageHook 日志（用于排查“卡在上一局/不同步/状态拿不到”等问题）。
- 详情页里有两种视图：
...
- 为了不打扰你：**只有在“校准/采样（兜底）”模式下**，弹窗才会显示“校准棋盘位置 / 校准&采样工具”等按钮；“复制校准参数 / 测缩放系数（采样）/ 管理采样点”等操作被整合进“采样点管理”页面里。
...
- 弹窗里点「校准/采样工具」→「打开测缩放系数（采样）」会打开一个采样页。
