...
2. 页面内 Hook 需要先拿到“游戏 API 对象”（特征：有 `getHolderData()`、`ejectState()`、`isOnline()` 这些方法）。
   ...
   - **强制重置（清缓存）**：如果你“退出对局再进/切模式”后发现状态卡在旧局面，弹窗里可以点「强制重置（清缓存）」。它会让 Hook **重新抓取游戏 API**（重装 Map tap + 立即扫描窗口候选），从而尽量切到“新的 API 对象”。
     - 优化：重置会尽量“无感恢复”——如果旧 API 还在正常更新（frame 在走），允许短暂兜底保留；如果旧 API 已经卡住（frame 不动），就会丢弃并重新抓新的。
   - **自动自救（防卡死）**：Hook 会监测 `frame`：
     - 如果发现 `frame` 连续一段时间完全不变（很像“旧 API 卡住/退出再进后还读上一局”），会自动 **ban 旧 API + 重新抓取**，并停止 keepalive 旧状态，避免 Content 侧一直显示上一局误导你。
   - **按 R 重开/切模式的恢复**：
     - 有些模式在“重开/准备阶段”会短暂拿不到完整 `board/current`。
     - Hook 会继续用 `raw.frame` 做心跳判断；如果在“刚刚还在对局中”的前提下，连续一段时间都拿不到完整状态，会自动重置捕获并重新抓 API，尽量保证重开后能跟上实时状态。
...
5. 最后用 `window.postMessage` 把状态发回 Content Script。
   ...
   - 调试：Hook 内部维护一个**环形日志缓冲**（最近约 240 条事件）。详情页提供「复制调试包」按钮，会把这些日志和当前状态一起打包，方便排查“卡在上一局/不同步/读不到状态”等问题。
