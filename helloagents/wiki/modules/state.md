...
2. 页面内 Hook 需要先拿到“游戏 API 对象”（特征：有 `getHolderData()`、`ejectState()`、`isOnline()` 这些方法）。
   ...
   - **强制重置（清缓存）**：如果你“退出对局再进/切模式”后发现状态卡在旧局面，弹窗里可以点「强制重置（清缓存）」。它会让 Hook 清掉 `window.__tbpGameApi` 并重新安装 Map tap，从而重新捕获新的 API 对象。
...
5. 最后用 `window.postMessage` 把状态发回 Content Script。
   - 重要：Hook 现在会做 **keepalive**（保活）——就算局面没变化，也会每隔大约 1 秒再发一次状态/错误提示。
   - 目的：避免在“双人练习准备阶段/长时间不落块/某些模式 frame 不变”时，Content 侧误判为“状态超时/Hook 卡死”。
