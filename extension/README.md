...
## 引擎说明（现在默认更像“对战”）
- 叠加提示默认使用 **Cold Clear（WASM，离线）** 来算“当前块推荐落点”（更像对战思路，会更愿意找 T-Spin/B2B）。
- 如果你的浏览器不支持 offscreen（或被策略禁用），或者 cold-clear worker 启动失败：实时叠加提示会暂时拿不到（不会回退到另一套逻辑，避免“悬浮和详情页不一致”）。
- 如果你看到 `wasm 初始化失败：...Content Security Policy...`：说明当前扩展页面 CSP 不允许编译 WASM。我们已在扩展里开启 `wasm-unsafe-eval`（需要重新加载扩展后生效）；如果你是公司/网吧电脑被策略禁了，那就只能改成“本地后端”模式（或换一台能跑 WASM 的环境）。
