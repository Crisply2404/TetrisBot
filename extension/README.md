...
- 你也可以切到 **本地 MisaMino（next5）**：
  - 你需要在本机启动 `misamino-server/server.js`（见 `misamino-server/README.md`）。
  - 如果扩展连不上本地 MisaMino：会自动回退到插件内置 **CC1** 兜底。
...
3. 点扩展图标：
   ...
   - 如果你发现“地形/块序对不上真实局面”（常见于退出对局再进/切模式后）：点「强制重置（清缓存）」让扩展清空缓存并重新抓取游戏 API
...
### 1.1）退出对局再进 / 切模式后，提示乱了（块/地形对不上）
这通常是页面里“游戏 API 对象”换了，但 Hook 还在读上一局的对象。

补充：Hook 现在会监测 `frame` 是否“卡住不动”，一旦疑似卡死会自动重置捕获并重新抓取（一般不需要你手动操作）。
补充：40L 按 `R` 重开、或某些模式的“准备阶段”，可能会短暂出现“状态还没准备好”；如果超过 2-3 秒还没恢复，扩展会自动尝试重抓（或你也可以手动点一次「强制重置（清缓存）」）。
...
排错（强烈推荐）：
- 点「打开详情（实时）」→ 右上角点「复制调试包」，把剪贴板内容发给我：里面包含当前状态、引擎信息、以及 pageHook 的最近日志，能快速定位到底卡在了哪里。

### 1.2）我想确认现在到底用的是 CC2 还是 CC1？
点扩展图标，状态里会显示一行：
- `引擎=cold-clear-2`：说明正在用本地 CC2
- `引擎=misamino`：说明正在用本地 MisaMino（next5）
...
如果你发现“窗口缩小后还是不准”，可以用「校准/采样工具」里的「打开测缩放系数（采样）」：按页面提示改窗口大小 + 校准 + 记录样本，最后复制 JSON 发给我，我会用这些样本把缩放适配做得更准。
